...
class ResultController extends Controller
{
    ...
    public function index() {
        ...
        $materialsOFteacher = DB::select("SELECT `teachers`.`id`, `teachers`.`fullName`, `materials`.`id` AS `materialID`, `materials`.`materialName` FROM `materialtoteachers`, `materials`, `teachers` WHERE `teachers`.`id` = `materialtoteachers`.`teacherId` AND `materials`.`id` = `materialtoteachers`.`materialId` AND `materials`.`status` = 0 AND `teachers`.`id` = 4;");
